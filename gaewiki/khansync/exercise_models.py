...
class Exercise(backup_model.BackupModel):
    ...
    tests = object_property.JsonProperty(default="[]")
    tests_comments = object_property.JsonProperty(default="[]")


    status = db.StringProperty(default="untested")
    ...
    def to_json(self, *args, **kwargs):
        return jsonify.jsonify(self, *args, **kwargs)

    @staticmethod
    def update_from_jsonlist(obj_list):
        ...
        """
        stored_exs = Exercise.all().fetch(99999)
        update_exs = [(stored_ex, obj) for stored_ex in stored_exs 
                        for obj in obj_list if stored_ex.name == obj['name']]

        updated_exs = [] 
        for idx, (ex, obj) in enumerate(update_exs):
            logging.error("Update %d/%d: %s" % (idx+1, len(update_exs), ex.name)) 
            ex.file_name = str(obj['file_name'])
            ex.pretty_display_name = str(obj["pretty_display_name"])
            ex.short_display_name = str(obj["short_display_name"])
            ex.prerequisites = obj["prerequisites"]
            ex.covers = obj["covers"]
            ex.v_position = int(obj["v_position"])
            ex.h_position = int(obj["h_position"])
            ex.seconds_per_fast_problem = float(obj["seconds_per_fast_problem"])
            ex.live = bool(obj["live"])
            ex.summative = bool(obj["summative"])
            ex.creation_date = datetime.datetime.now()
            ex.description = str(obj["description"])
            ex.tags = obj["tags"]
    ...
    #    lambda *args, **kwargs: "all_by_name_%s" % setting_model.Setting.exercises_last_update())
    def all_by_name():
        all_exs = Exercise.all().order("name").fetch(99999)
        return all_exs
